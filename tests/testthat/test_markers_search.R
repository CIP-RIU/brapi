...
test_that("Marker search basics.", {
  ...
  expect_equal(markers_search(con, "*", matchMethod = "wildcard") %>% ncol, 6)
})

test_that("Match method.", {
  expect_equal(markers_search(con, "a_01_10001", matchMethod = "exact") %>% nrow, 1)
  expect_equal(markers_search(con, "A_01_10001", matchMethod = "case_insensitive") %>% nrow, 1)
  expect_equal(markers_search(con, "?_01_1000?", matchMethod = "wildcard") %>% nrow, 2)

})

test_that("type", {
  expect_equal(markers_search(con, "*", matchMethod = "wildcard", type = "SNP") %>% nrow, 3)
