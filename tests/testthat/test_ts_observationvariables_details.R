context("ts observationvariables_details")

con <- ba_db()$testserver

test_that(" are present", {

  res <- ba_observationvariables_details(con = con)
  expect_that(nrow(res) == 5, is_true())

})

test_that(" out formats work", {

  res <- ba_observationvariables_details(con = con, rclass = "json")
  expect_that("json" %in% class(res), is_true())

  res <- ba_observationvariables_details(con = con, rclass = "list")
  expect_that("list" %in% class(res), is_true())

  res <- ba_observationvariables_details(con = con, rclass = "data.frame")
  expect_that("data.frame" %in% class(res), is_true())

})

