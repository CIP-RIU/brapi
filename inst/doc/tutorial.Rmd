...
To eaes local testing the package comes now with a localized simple testing server implementing the BrAPI v1 protocol. To start this server use in an R session:
...
connect() # the standard parameter will work connect to a running local mock server

crops() # which crops are available

calls() # which calls are available
