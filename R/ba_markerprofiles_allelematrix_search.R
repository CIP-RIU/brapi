...
#' @family genotyping
#' @export
ba_markerprofiles_allelematrix_search <- function(con = NULL, markerprofileDbId = "", markerDbId = "", expandHomozygotes = FALSE, unknownString = "-", 
    sepPhased = "|", sepUnphased = "/", format = "json", page = 0, pageSize = 10000, rclass = "tibble") {
    ...
    pallelematrix_search <- paste0(pallelematrix_search, pmarkerprofileDbId, pmarkerDbId, pexpandHomozygotes, psepPhased, psepUnphased, pformat, 
        ppageSize, ppage)
    ...
    if (nurl > 2000) {
        ...
        body <- list(expandHomozygotes = expandHomozygotes %>% tolower(), unknownString = unknownString, sepPhased = sepPhased, sepUnphased = sepUnphased, 
            format = format, page = page, pageSize = pageSize)
        body <- c(x1, x2, body)
        out <- try({
            pallelematrix_search <- paste0(brp, "allelematrix-search/?")
