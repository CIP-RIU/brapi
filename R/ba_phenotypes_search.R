...
#' @family brapicore
#' @export
ba_phenotypes_search <- function(con = NULL, germplasmDbIds = "any", observationVariableDbIds = "any", studyDbIds = "any", locationDbIds = "any", 
    programDbIds = "any", seasonDbIds = "any", observationLevel = "any", pageSize = 100, page = 0, rclass = "tibble") {
    ...
    try({
        body <- list(germplasmDbIds = germplasmDbIds, observationVariableDbIds = observationVariableDbIds, studyDbIds = studyDbIds, locationDbIds = locationDbIds, 
            programDbIds = programDbIds, seasonDbIds = seasonDbIds, observationLevel = observationLevel, pageSize = pageSize, page = page)
        ...
        if (rclass %in% c("tibble", "data.frame")) {
            ...
            trt <- as.data.frame(cbind(treatments.factor = rep("", nrow(out)), treatments.modality = rep("", nrow(out))), stringsAsFactors = FALSE)
