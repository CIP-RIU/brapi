...
#' @family genotyping
#' @export
ba_germplasmattributes_details <- function(con = NULL, germplasmDbId = "1", attributeList = "1", page = 0, pageSize = 10, rclass = "tibble") {
    ba_check(con, FALSE)
    ...
    brp <- get_brapi(con)
    germplasm_attributes_list <- paste0(brp, "germplasm/", germplasmDbId, "/attributes/?attributeList=", paste(attributeList, collapse = ","), "&page=", 
    ...
    try({
        ...
        ms2tbl <- function(res) {
            ...
            value <- NULL
            dateDetermined <- NULL
            res %>% as.character %>% enter_object("result") %>% spread_values(germplasmDbId = jnumber("germplasmDbId")) %>% enter_object("data") %>% 
                gather_array %>% spread_values(attributeDbId = jnumber("attributeDbId"), attributeCode = jstring("attributeCode"), attributeName = jstring("attributeName"), 
                value = jstring("value"), dateDetermined = jstring("dateDetermined")) %>% dplyr::select(germplasmDbId, attributeDbId, attributeName, 
