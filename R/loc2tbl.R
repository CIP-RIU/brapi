loc2tbl <- function(res, rclass, con) {
    ...
    field_list <- c("locationDbId", "locationType", "name", "abbreviation", "countryCode",
                    "latitude", "longitude", "altitude")
    dat <- complement_missing_fields(dat, field_list)
    ...
    if (has_add_cols) {
        ...
        df <- complement_missing_fields(dat, add_field_list)
