gp2tbl <- function(res) {
    ...
    res %>% as.character %>% enter_object("result") %>%
      ...
      spread_values(germplasmDbId = jnumber("germplasmDbId"),
        ...
        genus = jstring("genus"), species = jstring("species"),
        speciesAuthority = jstring("speciesAuthority"),
        subtaxa = jstring("subtaxa"), subtaxaAuthority = jstring("subtaxaAuthority"),
        acquisitionDate = jstring("acquisitionDate")) %>%
        enter_object("donors") %>% gather_array %>%
        spread_values(donors.donorAccessionNumber = jstring("donorAccessionNumber"),
